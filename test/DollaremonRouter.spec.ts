...
import IPancakePair from '@dollaremon/dollaremon-swap-core/build/IPancakePair.json'
...
describe('DollaremonRouter', () => {
  ...
  it('getAmountOut', async () => {
    ...
    await expect(router.getAmountOut(bigNumberify(0), bigNumberify(100), bigNumberify(100))).to.be.revertedWith(
      'DollaremonLibrary: INSUFFICIENT_INPUT_AMOUNT'
    )
  ...
  it('getAmountIn', async () => {
    ...
    await expect(router.getAmountIn(bigNumberify(0), bigNumberify(100), bigNumberify(100))).to.be.revertedWith(
      'DollaremonLibrary: INSUFFICIENT_OUTPUT_AMOUNT'
    )
  ...
  it('getAmountsOut', async () => {
    ...
    await expect(router.getAmountsOut(bigNumberify(2), [token0.address])).to.be.revertedWith(
      'DollaremonLibrary: INVALID_PATH'
    )
    const path = [token0.address, token1.address]
    const left = await router.getAmountsOut(bigNumberify(2), path)
    expect(left[0]).to.deep.eq(bigNumberify(2))
    expect(left[1]).to.deep.eq(bigNumberify(1))
  })
  ...
  it('getAmountsIn', async () => {
    ...
    const path = [token0.address, token1.address]
    const left = await router.getAmountsOut(bigNumberify(1), path)
    expect(left[0]).to.deep.eq(bigNumberify(1))
    expect(left[1]).to.deep.eq(bigNumberify(0))
  })
})
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    pair = new Contract(pairAddress, JSON.stringify(IPancakePair.abi), provider).connect(wallet)
  })
